...
class BenchmarkTestManager(BaseService):
    ...
    async def schedule_test_task(
        ...
        try:
            ...
            runner_id = slugify(input_data.run_name, lowercase=True)
            if runner_id in self.runner_service.get_all_runner_name():
                runner = self.runner_service.load_runner(
                    runner_id,
            ...
            self.logger.error("Failed to load or create runner - Error: %s, Stacktrace: %s", e, traceback.print_exc())
            raise ValueError("Failed to load or create runner due to invalid input.")
        except Exception as e:
            self.logger.error("Failed to load or create runner - Error: %s, Stacktrace: %s", e, traceback.print_exc())
            raise Exception(f"Unexpected error in core library - {e}")
